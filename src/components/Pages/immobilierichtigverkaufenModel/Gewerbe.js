      ...
      <thead>
        <tr>
          ...
          <th>
            <button
              type="button"
              onClick={() => requestSort("AreaOfCommercialProperty")}
              className={getClassNamesFor("AreaOfCommercialProperty")}
            >
              Area Of Commercial Property
            </button>
          </th>
          <th>
            <button
          ...
          <th>
            <button
              type="button"
              onClick={() => requestSort("UsableAreaOfBuilding")}
              className={getClassNamesFor("UsableAreaOfBuilding")}
            >
          ...
          <th>
            <button
              type="button"
              onClick={() => requestSort("buildingType")}
              className={getClassNamesFor("buildingType")}
            >
              Building Type
            </button>
          </th>
          <th>
            <button
              type="button"
              onClick={() => requestSort("commercialBuildingBuilt")}
              className={getClassNamesFor("commercialBuildingBuilt")}
            >
      ...
      <tbody>
        {items.map((item) => (
          <tr key={item._id}>
            ...
            <td>{item.AreaOfCommercialProperty}</td>
            <td>{item.SellYourProperty}</td>
            <td>{item.UsableAreaOfBuilding}</td>
            <td>{item.buildingType}</td>
            <td>{item.commercialBuildingBuilt}</td>
            ...
            <td>
              ...
              <button
                ...
                onClick={async () => {
                  await axios.put(
                    `https://eigenheim-backend.herokuapp.com/gewerbe/create-form-edit/${item._id}`,
                    { Note: input }
                  );
                  console.log(input);
                }}
...
export default function Form() {
  ...
  useEffect(() => {
    // GET request using axios inside useEffect React hook
    let data = axios
      .get("https://eigenheim-backend.herokuapp.com/gewerbe")
  ...
  return (
    <Auth>
      <div className="App">
        <h3 style={{ textAlign: "center" }}>Gewerbe</h3>
