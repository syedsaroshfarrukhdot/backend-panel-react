      ...
      <thead>
        <tr>
          ...
          <th>
            <button
              type="button"
              onClick={() => requestSort("IsPropertyDeveloped")}
              className={getClassNamesFor("IsPropertyDeveloped")}
            >
              Is Property Developed
            </button>
          </th>
          <th>
            <button
          ...
          <th>
            <button
              type="button"
              onClick={() => requestSort("BuldingOptions")}
              className={getClassNamesFor("BuldingOptions")}
            >
              Bulding Options
            </button>
          </th>
      ...
      <tbody>
        {items.map((item) => (
          <tr key={item._id}>
            ...
            <td>{item.IsPropertyDeveloped}</td>
            <td>{item.Area}</td>
            <td>{item.BuldingOptions}</td>
            ...
            <td>
              ...
              <button
                ...
                onClick={async () => {
                  await axios.put(
                    `https://eigenheim-backend.herokuapp.com/grundstuck/create-form-edit/${item._id}`,
                    { Note: input }
                  );
                  console.log(input);
                }}
...
export default function Form() {
  ...
  useEffect(() => {
    // GET request using axios inside useEffect React hook
    let data = axios
      .get("https://eigenheim-backend.herokuapp.com/grundstuck")
  ...
  return (
    <Auth>
      <div className="App">
        <h3 style={{ textAlign: "center" }}>Grundstuck</h3>
